...
int immediateValue(const Board& bd) {
    int winningSquares[16][4] = {
        ...
        { 0, 5, 10, 15 }, { 3, 6, 9, 12 }, { 0, 3, 12, 15}, {5, 6, 9, 10},
        {4, 7, 8, 11}, {1, 2, 13, 14}, {1,7,14,8}, {2,11,13,4}
    };

    // check each possible winning combo
    for (int i = 0; i < 16; i++) {
        int a = winningSquares[i][0];
        int b = winningSquares[i][1];
        int c = winningSquares[i][2];
        int d = winningSquares[i][3];

        if (bd.squares[a] == 'X' && bd.squares[b] == 'X' && bd.squares[c] == 'X' && bd.squares[d] == 'X') {
            return WINX;
        }
        else if (bd.squares[a] == 'O' && bd.squares[b] == 'O' && bd.squares[c] == 'O' && bd.squares[d] == 'O') {
            return WINO;
        }
    }
...
// Does the current board have four X's
bool isWinX(const Board & bd) {
    return immediateValue(bd) == 1;
}

// Does the current board have four O's 
